...
const API_BASE = "https://thai-stock-web-app.onrender.com/api"; 
...
function App() {
  ...
  const fetchAccount = useCallback(async () => {
    try {
      ...
      if (res.data.status === "success") {
        setAccountInfo(res.data.data[0] || { account_no: 'N/A', line_available: 0 }); 
      }
    } catch (err) { console.error("Account Fetch Error", err); }
  }, []);

  // ฟังก์ชันรวมสำหรับการโหลดข้อมูลครั้งแรก
  const initializeApp = useCallback(async () => {
    setLoading(true);
    await Promise.all([fetchData(), fetchPortfolio(), fetchAccount()]);
    setLoading(false); // เปลี่ยน loading เป็น false ตรงนี้!
  ...
  const getMarketStatus = () => {
    ...
    return (isMorning || isAfternoon) ? { label: "MARKET OPEN", color: "#22c55e" } : { label: "MARKET CLOSED (Break)", color: "#facc15" };
  };
  ...
  // หน้าจอ Loading ระหว่างรอข้อมูลครั้งแรก
  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100vw', height: '100vh', justifyContent: 'center', alignItems: 'center', backgroundColor: '#0f172a', color: 'white' }}>
      <RefreshCw size={48} style={{ animation: 'spin 2s linear infinite' }} />
      <p style={{ marginTop: '20px' }}>กำลังเชื่อมต่อ baipaiyo-E...</p>
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#0f172a', color: 'white', padding: '40px 20px', fontFamily: "'Inter', sans-serif" }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '15px', marginTop: '5px' }}>
              ...
              <div style={{ color: '#4ade80', display: 'flex', alignItems: 'center', gap: '5px' }}>
                <TrendingUp size={16} />
                <span>Cash: <strong>฿{Number(accountInfo?.line_available || 0).toLocaleString()}</strong></span>
              </div>
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>{currentTime.toLocaleTimeString('th-TH')}</div>
            <div style={{ color: marketStatus.color, fontSize: '0.85rem', fontWeight: 'bold' }}>● {marketStatus.label}</div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '30px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
            ...
            <div style={{ backgroundColor: '#1e293b', borderRadius: '24px', padding: '25px', border: '1px solid #334155' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
                ...
                <h3 style={{ margin: 0 }}>พอร์ตปัจจุบัน (baipaiyo-E)</h3>
              </div>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ color: '#64748b', fontSize: '0.8rem', borderBottom: '1px solid #334155' }}>
                    ...
                    <th style={{ textAlign: 'right', padding: '10px' }}>MARKET</th>
              ...
              {stocks.map((stock) => (
                <div key={stock.symbol} style={{ backgroundColor: '#1e293b', borderRadius: '24px', padding: '20px', border: '1px solid #334155' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                    ...
                    <span style={{ color: stock.pct?.includes('-') ? '#f87171' : '#4ade80', fontWeight: 'bold' }}>{stock.pct}</span>
                  </div>
                  <div style={{ fontSize: '2rem', fontWeight: '800', marginBottom: '15px' }}>฿{stock.price}</div>
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <button onClick={() => { setTradeData({ symbol: stock.symbol, side: 'BUY', volume: 100 }); setShowTradeModal(true); }} style={{ flex: 1, padding: '10px', borderRadius: '12px', backgroundColor: '#22c55e', color: 'white', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>BUY</button>
                    <button onClick={() => { setTradeData({ symbol: stock.symbol, side: 'SELL', volume: 100 }); setShowTradeModal(true); }} style={{ flex: 1, padding: '10px', borderRadius: '12px', backgroundColor: '#ef4444', color: 'white', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>SELL</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* AI Sidebar */}
          <div style={{ backgroundColor: '#1e293b', borderRadius: '28px', padding: '25px', border: '2px solid #38bdf8', height: 'fit-content' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
              <BrainCircuit color="#38bdf8" size={28} />
              <h3 style={{ margin: 0, color: '#38bdf8' }}>AI Insights</h3>
            </div>
            <button onClick={runAI} disabled={isAnalyzing} style={{ width: '100%', padding: '15px', borderRadius: '12px', backgroundColor: '#38bdf8', color: '#0f172a', fontWeight: 'bold', border: 'none', cursor: 'pointer', marginBottom: '20px' }}>
      ...
      {showTradeModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: '#1e293b', padding: '30px', borderRadius: '24px', width: '400px', border: '1px solid #38bdf8' }}>
            <h2 style={{ marginTop: 0 }}>ยืนยันคำสั่งซื้อขาย</h2>
            ...
            <input type="number" value={tradeData.volume} onChange={(e) => setTradeData({...tradeData, volume: parseInt(e.target.value)})} style={{ width: '100%', padding: '12px', borderRadius: '8px', marginBottom: '25px', backgroundColor: '#0f172a', color: 'white', border: '1px solid #334155' }} />
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button onClick={handleTrade} style={{ flex: 1, padding: '12px', borderRadius: '12px', backgroundColor: '#38bdf8', color: '#0f172a', fontWeight: 'bold', border: 'none' }}>ยืนยัน</button>
            </div>
          </div>
        </div>
      )}
